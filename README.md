...
Grids across the site set their column minimums with `minmax(min(…, 100%), 1fr)`, so they never push the page wider than the screen. Keep that pattern when adding new grids.
